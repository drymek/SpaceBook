...
func (s *ResponseSuite) TestSimpleResponse() {
	w := new(Writer)
	w.On("Write", []byte("{}\n")).Return(3, nil)
	err := httpx.EncodeResponse(logger.NewNullLogger())(context.TODO(), w, SimpleResponse{})
	s.NoError(err)
...
func (s *ResponseSuite) TestStatusCodeResponse() {
	...
	err := httpx.EncodeResponse(logger.NewNullLogger())(context.TODO(), w, NewStatusCodeResponse(http.StatusOK))
...
func (s *ResponseSuite) TestHeaderResponse() {
	...
	err := httpx.EncodeResponse(logger.NewNullLogger())(context.TODO(), w, NewHeaderResponse("Content-Type", "application/json"))
	s.NoError(err)
	w.AssertNumberOfCalls(s.T(), "Write", 1)
...
func (s *ResponseSuite) TestEmptyResponse() {
	w := new(Writer)
	err := httpx.EncodeResponse(logger.NewNullLogger())(context.TODO(), w, EmptyResponse{})
	s.NoError(err)
